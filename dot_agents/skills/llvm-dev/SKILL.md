...
- Prefer **fewer false positives** over more reports: better to miss a bug than train users to ignore the checker.
- Emit path notes / diagnostics only when they clarify a real report; avoid noise on unrelated bug paths.
- Match the style already used in the file you touch (LLVM “golden rule”).
